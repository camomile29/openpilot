...
MapPanel::MapPanel(QWidget* parent) : QWidget(parent) {
  ...
  // Current route
  {
    ...
    QLabel *title = new QLabel("현재 목적지");
    title->setStyleSheet("font-size: 55px");
    current_layout->addWidget(title);

    current_route = new ButtonControl("", "삭제");
  ...
  // Recents
  QLabel *recents_title = new QLabel("최근 목적지");
  ...
  QWidget * no_prime_widget = new QWidget;
  {
    ...
    QLabel *signup_header = new QLabel("내비게이션 베타를 시도해보세요");
    ...
    QLabel *signup = new QLabel("콤마 프라임 구독으로 턴바이턴 길찾기 등을 확인하세요. 지금 가입하세요: https://connect.comma.ai");
...
void MapPanel::clear() {
  ...
  home_address->setText("집이 설정되지 않았습니다 위치를 설정하세요");
  ...
  work_address->setText("회사가 설정되지 않았습니다 위치를 설정하세요");
...
void MapPanel::parseResponse(const QString &response, bool success) {
  ...
  if (!has_recents) {
    QLabel *no_recents = new QLabel("최근 목적지가 없습니다");
